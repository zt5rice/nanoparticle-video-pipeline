...
| `Fakhri et al. - 2010 - Brownian Motion of Stiff Filaments in a Crowded En.pdf` + `Fakhri-SOM.pdf` | Fakhri et al. 2010，拥挤环境中刚性丝的布朗运动（Science）+ SOM | **parallel/perp MSD 方法来源**：质心位移分解为平行/垂直于 running time-averaged reptation tube 的分量（SOM 附录，`R_Θτ` 公式）；作者的 MATLAB `MATLAB code` 即实现该做法 |
...
`features.msd_parallel_perpendicular` 实现的是 **Fakhri 2010** 方法（即作者 MATLAB `MATLAB code`
...
- 叠加视频：`run_pipeline.py --overlay [--overlay-format mp4|avi|gif|png]` 把追踪轨迹/朝向
  画到输入视频上，写出 `output/tracking_overlay.<ext>`（对应 MATLAB `bdmovieSWNT`
  moviemaker；`--resume` 模式无输入帧，自动跳过）。
